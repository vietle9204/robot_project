...
#         # Tích phân vị trí trong global frame
#         x_new = self.z_k[0,0] + self.imu_vx*dt + 0.5*a_x_global*dt**2
#         y_new = self.z_k[1,0] + self.imu_vy*dt + 0.5*a_y_global*dt**2
...
#         # Predict
#         # forward kinematics:  x_k_f = A x_k + B u_k 
#         x_k_f = self.A_k.dot(self.x_k) + self.G_k.dot(self.u_k)   # (3,1)
#         x_k_f[2,0] = angle_normalize(float(x_k_f[2,0]))

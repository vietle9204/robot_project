...
class UKFSLAM(Node):
    ...
    def association(
        ...
        for i, (z_obs, R_obs) in enumerate(features):
            ...
            candidate_ids = self.kdtree.query_ball_point(
                [gx, gy],
...
#         # self.get_logger().info('Data Buffer Node Started')
#         # self.odom_sub = Subscriber(self, Odometry, self.odom_topic, qos_profile=qos2)
#         # self.scan_sub = Subscriber(self, LaserScan, self.scan_topic, qos_profile=qos)
#         # self.ts = ApproximateTimeSynchronizer(
#         #     [self.odom_sub, self.scan_sub],
...
#         # self.get_logger().info(f"Received odometry: time={msg.header.stamp.sec}.{msg.header.stamp.nanosec}, pose=({msg.pose.pose.position.x:.4f}, {msg.pose.pose.position.y:.4f}, {yaw_from_quaternion(msg.pose.pose.orientation):.4f})")
        
        
#     def odom_process(self, msg: Odometry):
...
#             self.publish_pose(odom_msg.header.stamp)
#             self.get_logger().info("Processing odometry message.")
#             return
#         else:
#             odom_msg = self.odom_buffer[0]
#             scan_msg = self.scan_buffer[0]
...
#         # --- Sigma points ---
#         w_m, w_c, sigma_points = self.w_m, self.w_c, self.sigma
#         # if sigma_points is None:
#         #     self.generate_sigma_points(self.x, self.P)
